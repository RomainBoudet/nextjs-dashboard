...
export async function fetchLatestInvoices() {
  ...
  try {
    const data = await db.query<LatestInvoiceRaw>(`
...
export async function fetchInvoiceById(id: string) {
  try {
    const data = await db.query<InvoiceForm>(`
      SELECT
        invoices.id,
...
export async function fetchCustomers() {
  try {
    const data = await db.query<CustomerField>(`
      SELECT
        id,
        name
      FROM customers
...
export async function fetchFilteredCustomers(query: string) {
  try {
    const data = await db.query<CustomersTableType>(`
		SELECT
		  customers.id,
		  customers.name,
		  customers.email,
